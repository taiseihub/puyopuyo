...
#前回からの変更点
#盤面取得を関数化, 取得する側の盤面のみ変換にかける
#保存形式をcsv→jsonへ, jsonではclassが扱えないため盤面はNone or intで保存

video = cv2.VideoCapture('s30424.mp4')
filename = "s30424"
...
def check_score():
    ...
    template = cv2.imread('pngs\\score_template.png')
...
while video.isOpened():
    ...
    #field, ぷよが設置されているか確認
    if check_field():
        #fieldが100tick連続で検出できない場合、次の試合に移動したと判断する
        if frame_count >= 100 and len(field_dict["fields"]) != 0:
            field_dict["game_no"] = game_no
    ...
    if (check_s[0] or check_n[0]) and not skip1 and tick1 < tick:
        field1 = get_field(frames[0])# 自分の盤面
        field_dict["fields"][0].append({"tick":tick, "rensa": check_s[0],"field":field1})#rensaがTrue→連鎖で取得した
        tick1 = tick + 18 # 18tick内に新たにぷよが設置されたと判定された場合はおそらく間違っているため除外する
        cv2.imshow("test", image) # 確認用
        
    if (check_n[1] or check_s[1]) and not skip2 and tick2 < tick:
        field2 = get_field(frames[1]) # 相手の盤面
        field_dict["fields"][1].append({"tick":tick, "rensa":check_s[1], "field":field2})
        tick2 = tick + 18
